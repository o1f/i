jQuery(function($){
	...
	$('body').on('change', 'select#page_key', function(){
		...
		if(typeof page_key_fields == 'undefined'){
			page_key_fields	= {};
		}
		
		$(':input').each(function(){
			if($(this).data('page_key')){
				var tr_item	= $('#tr_'+$(this).attr('id'));

				$.each($(this).data('page_key').split(','), function(index, key){
					page_key_fields[key]	= tr_item;
				});
		...
		if(!$.isEmptyObject(page_key_fields) && isset(page_key_fields[page_key])){
			$('#tr_'+$(this).attr('id')).after(page_key_fields[page_key]);
		}
	});
